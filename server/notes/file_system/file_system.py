...
class FileSystemNotes(BaseNotes):
    ...
    def __init__(self, storage_path: str = None):
        if storage_path:
            ...
            try:
                ...
                self.storage_path = system_config.get_current_storage_path()
            except Exception:
    ...
    def _index_path(self):
        new_path = os.path.join(self.storage_path, ".siwan_index")
        legacy_path = os.path.join(self.storage_path, ".flatnotes")
    ...
    def _path_from_title(self, title: str) -> str:
        ...
        return os.path.join(self.storage_path, clean_title + MARKDOWN_EXT)
    ...
    def _add_note_to_index(
        ...
        norm_filename = (note.title + MARKDOWN_EXT).replace("\\", "/")
    ...
    def _list_all_note_filenames(self) -> List[str]:
        """Return a list of all note filenames, scanning both root and subdirectories."""
        results = []
        skip_dirs = {".siwan_index", ".flatnotes", "attachments", ".git", "__pycache__", ".well-known"}
        for root, dirs, files in os.walk(self.storage_path):
            dirs[:] = [d for d in dirs if d not in skip_dirs and not d.startswith(".")]
            for f in files:
                if f.endswith(MARKDOWN_EXT) and not f.startswith("."):
                    full_p = os.path.join(root, f)
                    rel = os.path.relpath(full_p, self.storage_path)
    ...
    def _strip_ext(filename):
        ...
        return os.path.splitext(filename.replace("\\", "/"))[0]

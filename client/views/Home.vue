<template>
  <div class="flex h-full w-full justify-center px-3 sm:px-4">
    <div class="flex w-full max-w-[540px] flex-1 flex-col items-center pt-8 sm:pt-[15vh] md:pt-[20vh]">
      <Logo class="mb-6 flex justify-center" />
      <SearchInput class="mb-5 w-full shadow-[0_0_20px] shadow-theme-shadow" />
      <LoadingIndicator
...
onMounted(() => {
  init();
  window.addEventListener("siwan-storage-changed", init);
  window.addEventListener("siwan-auth-changed", init);
});

onBeforeUnmount(() => {
  window.removeEventListener("siwan-storage-changed", init);
  window.removeEventListener("siwan-auth-changed", init);
});
</script>
